...
(function () {
    ...
    w2grid.prototype = {
        ...
        buttons: {
            ...
            'add'      : { type: 'button', id: 'w2ui-add', caption: w2utils.lang('회원생성'), hint: w2utils.lang('register'), icon: 'w2ui-icon-plus' },
            'add2'      : { type: 'button', id: 'w2ui-add2', caption: w2utils.lang('계좌목록'), hint: w2utils.lang('account list')},
            'reply'      : { type: 'button', id: 'w2ui-reply', caption: w2utils.lang('Q&A목록'), hint: w2utils.lang('q&a list')},
            'secure'      : { type: 'button', id: 'w2ui-secure', caption: w2utils.lang('보안카드 발급'), hint: w2utils.lang('secure card')},
            'account'      : { type: 'button', id: 'w2ui-account', caption: w2utils.lang('계좌생성'), hint: w2utils.lang('create account')},
            'edit'     : { type: 'button', id: 'w2ui-edit', caption: w2utils.lang('회원수정'), hint: w2utils.lang('Edit selected record'), icon: 'w2ui-icon-pencil', disabled: true },
            'delete'   : { type: 'button', id: 'w2ui-delete', caption: w2utils.lang('회원삭제'), hint: w2utils.lang('Delete selected records'), icon: 'w2ui-icon-cross', disabled: true },
            'save'     : { type: 'button', id: 'w2ui-save', caption: w2utils.lang('저장'), hint: w2utils.lang('Save changed records'), icon: 'w2ui-icon-check' }
        },
        ...
        initToolbar: function () {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                if (this.show.toolbarSearch && (this.show.toolbarAdd || this.show.toolbarAdd2 || this.show.toolbarReply || this.show.toolbarSecure || this.show.toolbarAccount || this.show.toolbarEdit || this.show.toolbarDelete || this.show.toolbarSave)) {
                    this.toolbar.items.push({ type: 'break', id: 'w2ui-break1' });
                }
                if (this.show.toolbarAdd) {
                    this.toolbar.items.push($.extend(true, {}, this.buttons['add']));
                }
                if (this.show.toolbarAccount) {
                    this.toolbar.items.push($.extend(true, {}, this.buttons['account']));
                }
                if (this.show.toolbarAdd2) {
                    this.toolbar.items.push($.extend(true, {}, this.buttons['add2']));
                }
                if (this.show.toolbarSecure) {
                    this.toolbar.items.push($.extend(true, {}, this.buttons['secure']));
                }
                if (this.show.toolbarReply) {
                    this.toolbar.items.push($.extend(true, {}, this.buttons['reply']));
                }
                ...
                if (this.show.toolbarSave) {
                    if (this.show.toolbarAdd || this.show.toolbarAdd2 || this.show.toolbarReply || this.show.toolbarSecure || this.show.toolbarAccount || this.show.toolbarDelete || this.show.toolbarEdit) {
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        ...
                        case 'w2ui-add2':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'add2', recid: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-reply':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'reply', recid: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-secure':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'secure', recid: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-account':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'account', recid: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-edit':
                            var sel     = obj.getSelection();
...
(function ($) {
    ...
    var w2field = function (options) {
        ...
        this.onAdd2       = options.onAdd2       || null;
        this.onReply       = options.onReply       || null;
        this.onAccount       = options.onAccount       || null;
            ...
            switch (this.type) {
                ...
                case 'enum':
                    defaults = {
                        ...
                        onAdd2           : null,          // when an item is added
                        onReply           : null,          // when an item is added
        ...
        render: function (box) {
            ...
            if (typeof this.tabs.render !== 'function') {
                ...
                this.tabs.on('click', function (event) {
                    obj.go(this.get(event.target, true));
                });
            }
