...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#next").click(function(){
			location.href="questionsearch.bank?questionsearch=${param.questionsearch}&page=${listVO.pagingBean.endPageOfPageGroup+1}";
		});
...
<c:choose>
	<c:when test="${empty listVO.list}">
		<p id='p'>검색결과가 존재하지 않습니다</p>
	</c:when>
	<c:otherwise>
		<table id="questiontable">
		<tr><td>카테고리</td><td>번호</td><td>제목</td><td>조회수</td></tr>
		<%int num=1; %>
		<c:forEach items="${listVO.list}" var="list">
			<tr>
				<td>${list.section}</td>
				<td><%=num%></td>
				<td><a  id="a" href="questionshowcontents.bank?no=${list.no}">${list.title }</a></td>
				<td>${list.hits }</td>
			</tr>
			<%num++; %>
		</c:forEach>
